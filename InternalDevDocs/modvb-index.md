...
- **本文件用途**：为 ModVB / VBScript.NET 各提案的 meeting agent 提供 `modvb` 目录（VBScript.NET 自己的语言扩展提案库）的浓缩背景与文件地图，节省逐个读库的 token。
- **如何使用**：
  1. 先读「一、ModVB 现实方向摘要」快速建立世界观；
  2. 若某主题与手头提案相关，到「二、关键文件索引」用 Windows 反斜杠路径 + Grep 关键词深挖原文（提案）与决议（会议纪要）；
  3. 手头提案的「VBScript.NET 应对」应参考 **`modvb\decisions.md`（VBScript.NET 设计决策记录）** 的 M1–M8 与 D1–D4；C# 事实见 `..\csharplang-index.md`，VB 主线事实见 `..\vblang-index.md`。
- **来源目录**：`modvb\`。结构：**102 份提案**（`proposals\` 74 份 active + `proposals\inactive\` 28 份实验性）+ **102 篇会议纪要**（`meetings\` 74 + `meetings\inactive\` 28，**与提案 1:1 对应**：`meeting-<slug>` ↔ `proposal-<slug>`，2026-08 生成，每篇含 RESOLUTION/三态判定/五维评价/「附录：C# 生态与互操作考量」）。
- **提案源头**：`AnthonyDesign_wordpress.txt`（Anthony D. Green 博客《The Agenda – A Visual Guide》全文，2026-04-27，HTML 格式，18 章 + Wrap-up）。`proposals\README.md` 已给出按原文章节分组的 102 提案索引（编号 01–102），本索引以其为骨架。
...
- **关键文件**：`modvb\decisions.md`（M1–M8 决策映射 + D1–D4 决策修正，**VBScript.NET 侧唯一权威**）、`modvb\evaluation-standard.md`（五维评价标准 + LDM 追问清单）、`modvb\meetings\PROGRESS.md`（会议生成台账，G0–G25/C0–C25 分组）。
...
- **inactive/ 即原文第 18 章**「Needs more bake time」（还需要时间酝酿）的实验性想法（28 份，编号 75–102），多数仅一句观察、无语法无设计，会议统一判 Table（保持 inactive），个别（scripting-interpreted、method-level-imports 等）Consider。
...
- `modvb\decisions.md` M1–M8 已给出八条「C# 现实方向 → VBScript.NET 应对」映射，本索引提案表逐行标注命中编号：
...
- 通配符 Lambda `*.Member`（07 Consider）、简写属性/事件（08 Active，仅 `Return` 表达式体落地）、Markdown 文档注释（09 Table/Consider）、`#Ignore Warning`（10 Table）、隐式行继续（11 Consider）、杂项修复（12 拆分 1/2/4/7 Active）。
...
路径均为 Windows 反斜杠（相对 `modvb\`）；关键词供 Grep 定位。状态 `active`=提案在 `proposals\` 根、`inactive`=在 `proposals\inactive\`；会议纪要同名且在 `meetings\`（inactive 在 `meetings\inactive\`）。三态判定取自对应会议纪要「三态判定」小节。
...
| `proposals\proposal-wildcard-lambdas.md` | `*.Member` 通配符取代单参纯成员访问 lambda | 通配符 Lambda, 表达式树等价, EF 配置 | inactive | Consider | M2/M8 |
...
| `proposals\proposal-markdown-doc-comments.md` | 文档注释改用 Markdown 小节/@引用/围栏示例 | 文档注释, Markdown, XML doc | inactive | Table/Consider | M8 |
| `proposals\proposal-ignore-warning-directive.md` | 单行 `#Ignore Warning` 按错误号关段内警告 | #Ignore Warning, 错误号抑制 | inactive | Table | M8 |
...
| `proposals\proposal-set-statement.md` | 显式 Set 赋值语句，支持复合赋值/多重赋值/解构赋值 | Set, 复合赋值, 解构赋值 | active | Table | M4/M8 |
| `proposals\proposal-select-case-enhancements.md` | Select Case 支持类型/形状/恒等/Like/集合成员匹配 | Select Case, TypeOf, ShapeOf | inactive | Table | M4/M8 |
...
| `proposals\proposal-throw-inference.md` | 裸 Throw 依条件形态推断异常类型，简化参数校验 | Throw, ArgumentNullException | inactive | Table | M8 |
...
| `proposals\proposal-using-synclock-enhancements.md` | Using/SyncLock 块内 Catch/Finally、头解构、按名 Dispose | Using, SyncLock, 解构头, Dispose | active | Table（解构头 Active） | M5/M8 |
| `proposals\proposal-postfix-casting.md` | 后置转换 `expr(As Type)`，链式转换更可读 | 后置转换, As Type, CType | inactive | Table | M2 |
...
| 路径 | 一句话要点 | 关键词 | 状态 | 三态判定 | Mx |
|---|---|---|---|---|---|
| `proposals\proposal-xaml-literals.md` | `<?xaml?>` 内联 XAML 声明，编译器降级为 UI 对象树 | XAML 字面量, {Binding}, 声明式 UI | inactive | Table（A 附复活信号） | M5/M8 |
| `proposals\proposal-embedded-vb-mode.md` | `<?vb?>` 开启 VB 解析模式，HTML 即 XML 字面量 | 嵌入解析模式, XML 字面量, 隐式 Return | inactive | Table（C source-gen Consider） | M5/M8 |
| `proposals\proposal-xml-schema-types.md` | 语言级 `<geo:Address>` 类型标注恢复 XML IntelliSense | XML 轴属性, XSD, 注释型变体 | inactive | Table（注释变体 Consider） | M2/M5/M8 |
| `proposals\proposal-json-literals.md` | `{}` JSON 字面量做目标类型化创建与 `&=` 写给 writer | JSON 字面量, JsonObject, System.Text.Json | inactive | Consider（数据字面量） | M4/M5/M8 |
| `proposals\proposal-json-pattern-matching.md` | 用 JSON 形状做模式匹配断言与分发，配 JSON Schema 标注 | ShapeOf, JSON 模式匹配, JSON Schema | active | Table（形状模式 Consider） | M4/M5/M8 |
| `proposals\proposal-string-pattern-matching.md` | 插值字符串当模式，`{message}` 占位符抽取片段绑定变量 | 插值逆运算, 字符串模式, Like | active | Table（B 子集 Consider） | M4/M6/M8 |
| `proposals\proposal-interpolated-string-optimization.md` | 插值字符串优化：合并相邻、常量孔折叠、StringBuilder `&=` | 插值字符串, 常量孔折叠, StringBuilder | active | Table（B→Active） | M6/M8 |
| `proposals\proposal-string-narrowing-conversions.md` | 字符串窄化转换：Parse 常量、TryParse 配 ShapeOf、转枚举 | 字符串窄化, Parse, TryParse, 枚举 | inactive | Table（枚举窄化 Consider） | M3/M4/M8 |
| `proposals\proposal-shapeof-pattern-matching.md` | Select Case 按运行时类型分发并绑定强类型变量 | ShapeOf, Case pn As Type, 类型模式 | inactive | Table（归家族 Phase 1 Active） | M4 |
| `proposals\proposal-user-defined-pattern-methods.md` | 带 Out 参数返回 Boolean 的函数作模式，判定+取值一次完成 | 用户定义模式方法, Out, Pattern 标记 | inactive | Table（归家族 Phase 2 Active） | M4 |
| `proposals\proposal-named-patterns.md` | 函数即模式，以解构语法递归匹配对象形状 | 命名模式, 解构语法, 工厂镜像 | inactive | Table（概念 Active） | M3/M4 |
...
| `proposals\proposal-module-enhancements.md` | 模块可泛型/嵌套，默认不提升成员，StandardModule 回退 | 泛型模块, StandardModule, hoist | inactive | Table（泛型非提升 Consider） | M8 |
| `proposals\proposal-method-level-imports.md` | 方法级 Imports 收窄命名空间/Shared 成员导入作用域 | 方法级Imports, 作用域收窄, XML命名空间 | inactive | Consider | M8 |
| `proposals\proposal-bit-enum.md` | 位枚举专用语法：成员自动 2 的幂、逗号掩码、位段 | Bit Enum, 掩码, 位段, flags | inactive | Table（声明侧 Consider） | M2/M8 |
...
| `proposals\proposal-pipeline-operator.md` | 管道运算符 `->` 改写嵌套调用为从左到右数据流 | 管道运算符, 嵌套调用, It 记号 | inactive | Reject（窄方案 Consider） | M8 |
| `proposals\proposal-range-expressions.md` | `1 To 10 Step 2` 范围表达式字面量作 For Each 迭代源 | 范围表达式, To/Step, 等差序列 | active | Table（仅迭代源 Consider） | M8 |
...
| `proposals\proposal-query-comprehensions.md` | For Each 内 Include 关联加载理解 + 六个未展示项 | Include, Skip Until, Left Join | inactive | Table | M5/M8 |
| `proposals\proposal-insert-update-delete-expressions.md` | 声明式对数据源增删改的 Insert/Update/Delete 表达式 | Insert, Update, Delete, DML, DTO | inactive | Table | M2/M5/M8 |
...
| `proposals\proposal-in-notin-operators.md` | In/NotIn 运算符映射 Contains 做成员测试与区间判定 | In, NotIn, Contains, 范围表达式 | inactive | Table | 无 |
| `proposals\proposal-any-pseudotype.md` | Any 伪类型做逐变量显式晚期绑定替代 Object | Any, 伪类型, 晚期绑定, Variant | active | Consider | M2 |
| `proposals\proposal-typeless-declarations.md` | 无类型声明默认改用 Any 动态（渐进类型化） | Any, 无类型声明, 渐进类型化 | inactive | Table（收进脚本模式） | M2 |
| `proposals\proposal-default-methods.md` | Default 方法使对象可像函数调用（sp()=sp.Invoke()） | Default Sub Invoke, 可调用对象 | active | Consider | M7 |
| `proposals\proposal-async-sub.md` | Async Sub 可声明 Task/ValueTask 返回类型并配置化默认 | Async Sub, As Task, ValueTask | inactive | Reject | M2/M5 |
| `proposals\proposal-agile-async.md` | Agile Async 修饰符免写 ConfigureAwait(False)，省略中间 Await | Agile Async, ConfigureAwait, Await 省略 | inactive | Table | M2/M8 |
| `proposals\proposal-require-await-call.md` | 异步调用必须 Await 或赋给任务对象，fire-and-forget 用 Call | Await, Call, fire-and-forget | active | Reject(语言)/Active(分析器) | M8 |
...
| `proposals\proposal-async-event.md` | Async Event 声明异步事件，handler 可异步执行可被等待 | Async Event, RaiseEvent, AsyncEventHandler | inactive | Reject | M8 |

### 提案（proposals）— 章 12–17 Null/声明式/类型系统/继承/性能/运行时（56–74）
| 路径 | 一句话要点 | 关键词 | 状态 | 三态判定 | Mx |
|---|---|---|---|---|---|
| `proposals\proposal-null-literal.md` | Null 字面量统一空引用与空值，If() 推断强制提升可空 | Null, Nothing, 可空推断, NRT | inactive | Table | M2/M5 |
| `proposals\proposal-null-equality-operators.md` | `?=`/`?<>` 二值逻辑相等，双 null 相等，等价 IS DISTINCT FROM | 二值逻辑, ?=, 三值逻辑, T-SQL | active | Table | M8 |
| `proposals\proposal-null-coalescing.md` | 后置空合并 `??` 与空指示符 `?`（`True?`/`String?`/`lookup()?`） | 空合并, ??, 空指示符, NRT | active | Table（True? Consider） | M8 |
| `proposals\proposal-do-nothing.md` | 显式空操作语句 Do Nothing，为需语句但无事可做的位置占位 | Do Nothing, 空操作占位 | inactive | Table | 无 |
| `proposals\proposal-null-safe-behaviors.md` | `?.` 扩展到语句层，对象为 null 时循环/等待/赋值/事件整体跳过 | 空传播, 空条件赋值, AddHandler | inactive | Reject（两窄子集 Consider） | M8 |
| `proposals\proposal-smart-attributes.md` | 可组合特性注入属性/事件访问器行为，免源生成器 | PropertyHandlerAttribute, 特性注入 | inactive | Table | 无 |
| `proposals\proposal-replacement-modifiers.md` | Replaceable/Replaces/MustReplace 声明-替换协作契约 | Replaceable, Replaces, MustReplace | inactive | Table（MustReplace Consider） | M2/M5 |
...
| `proposals\proposal-array-pseudotype.md` | 数组伪类型 `Array(Of T)` 拼写与 `New Array(Of T)(size)` 实例化 | Array(Of T), newarr, off-by-one | inactive | Table（按大小实例化 Consider） | M2/M8 |
...
| `proposals\proposal-override-signature-relaxation.md` | 覆写返回类型协变为派生类型（零新语法） | 协变返回类型, Overrides, widening | inactive | Consider（范围收缩挂起） | M7 |
...
| `proposals\proposal-interface-delegation.md` | 接口实现委托给字段/属性，字段 Implements 接口即转发全部成员 | 接口委托, 组合优于继承, 转发方法 | inactive | Consider | M7 |
...
| `proposals\proposal-runtime-library.md` | 运行时库八条改进：转换性能、异常质量、晚期绑定、My | 运行时库, 内建转换, 晚期绑定 | active | Table（性能/二进制 Active） | M5/M8 |

### 提案（proposals\inactive）— 章 18 实验性想法（75–102，28 份）
> 均保持 inactive（Table），多数无语法仅登记；仅标注例外。会议纪要同名于 `meetings\inactive\`。
...
| `proposals\inactive\proposal-scripting-interpreted.md` | 脚本化记录（「Of course.」）；真解释器 Reject，走脚本宿主+REPL | 脚本宿主, REPL, 顶层代码 | **Consider** | M5 |
...
- **命名规则**：`meetings\meeting-<slug>.md` ↔ `proposals\proposal-<slug>.md`；inactive 同理在 `meetings\inactive\`。提案表任一行的会议路径 = 提案路径 `proposals\` → `meetings\`、`proposal-` → `meeting-`。
...
- **代表会议（按主题深挖起点）**：
  ...
  - 模式匹配家族：`meetings\meeting-shapeof-pattern-matching.md`、`meetings\meeting-user-defined-pattern-methods.md`、`meetings\meeting-select-case-enhancements.md`
  - 动态/类型化出口：`meetings\meeting-any-pseudotype.md`、`meetings\meeting-postfix-casting.md`、`meetings\meeting-typeless-declarations.md`
  - 脚本化核心：`meetings\meeting-top-level-code.md`、`meetings\inactive\meeting-scripting-interpreted.md`
  - 声明式/生成器：`meetings\meeting-smart-attributes.md`、`meetings\meeting-replacement-modifiers.md`、`meetings\meeting-semantic-preprocessing.md`
  - Null 安全家族：`meetings\meeting-null-safe-behaviors.md`、`meetings\meeting-null-coalescing.md`、`meetings\inactive\meeting-nullable-reference-types.md`
  - 异步：`meetings\meeting-try-enhancements.md`（Await in Catch/Finally）、`meetings\meeting-async-iterator.md`、`meetings\meeting-require-await-call.md`
- **生成台账**：`meetings\PROGRESS.md`（G0–G25 会议生成分组 + C0–C25 C# 附录分组，已 102/102 完成）。

### 元文件（modvb 根 / proposals 根）
| 路径 | 一句话要点 |
|---|---|
| `modvb\decisions.md` | **VBScript.NET 侧决策唯一权威**：D1–D4 决策修正（RefStructHelper 移植、NativeAOT 桥、postfix-casting 语义、P1 优先级判定）+ M1–M8 八条「C# 现实 → 应对」映射 |
| `modvb\evaluation-standard.md` | 五维评价框架（效果/特性/品质/属性/炼金成分）× 5 级评分 + LDM 追问清单 + 强提案品质清单/弱提案红旗 |
| `modvb\proposals\README.md` | **102 提案按原文章节 1–18 分组的现成索引**（编号 01–102 + 一行摘要），本索引骨架 |
...
> 原「三、对 ModVB / VBScript.NET 的含义（M1–M8 + D1–D4）」已独立成文，见 **`modvb\decisions.md`**（VBScript.NET 设计决策记录）。该文件同时记录决策修正（RefStructHelper 解法、NativeAOT 桥、postfix-casting 语义、P1 优先级判定规则）。本索引只保留 ModVB 事实与文件地图；决策与应对以 `decisions.md` 为准。
...
- 本索引中已核实可引用的原文与出处（供 meeting agent 直接使用）：
  ...
  - 「We're proud not to do anything」→ 多处（源自主线 2014-02-17；如 `modvb\meetings\meeting-do-nothing.md` RESOLUTION 5、`modvb\meetings\meeting-wildcard-lambdas.md` RESOLUTION 7、`modvb\meetings\inactive\meeting-named-pattern-inputs.md`）。
  - 「Fantastic idea, and too hard to do.」→ `modvb\meetings\meeting-xaml-literals.md`（第 12 节）、`modvb\meetings\inactive\meeting-structure-constraint-overloading.md`（RESOLUTION 2）。
  - 「Control flow would be altered by a very subtle character.」→ 多处（主线 #167 `Return?` 判词；如 `modvb\meetings\meeting-null-safe-behaviors.md`、`modvb\meetings\inactive\meeting-chained-ternary.md`、`modvb\meetings\meeting-agile-async.md`）。
  - 「We will almost never make breaking changes to Visual Basic」→ `modvb\meetings\meeting-require-await-call.md`（权衡，引 2018-06-13 主线原则）。
  - 「Treat it as an optimization, not a feature.」→ 多处（`modvb\meetings\meeting-try-enhancements.md`、`modvb\meetings\meeting-interpolated-string-optimization.md`、`modvb\meetings\meeting-runtime-library.md`、`modvb\meetings\inactive\meeting-structure-constraint-overloading.md`）。
