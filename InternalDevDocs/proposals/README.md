...
| 22 | `inactive/proposal-top-level-implicit-shared.md` | 脚本/提交类**顶层成员默认共享**（不写 `Shared` 也按共享处理；把 `spec-scripting-dialect.md:60` 的语义反过来）：**容器种类不变**——`TypeKind`/`DeclarationKind`/`SourceTypeFlags` 全不动，不引入标准模块语义（与已两次否决的「改容器种类」划清界限）。**方向不采纳、维持 Table**：核心论证被推翻——Summary 的「出货面上没有可观察差异」是**全称主张**，举不出证据且有一条方向明确的源码级反例（`spec:64` 的「field initializers and global statements **in source order**」在该字段上失守，`.vbx` **单次运行内**即可见：写在 `Dim x = 5` 之前的读取由 `0` 变 `5`；`SourceMemberContainerTypeSymbol.vb:1571-1573` 的 syntax-order 断言 + `:2633` 顶层语句入实例桶 + `MethodCompiler.vb:695-699` 静态桶→`.cctor`）；且 §6 的机制句**方向写反**（共享初始化器今天就在 `.cctor`，默认共享是把顶层 `Dim` 的初始化器**搬出** `<Initialize>` 的源码序——那是已被否的候选「丙」的机制；`SynthesizedConstructorBase.vb:192` + `MethodSymbol.vb:517-521` 使共享构造器 `IsScriptConstructor` 恒假）。另：与 `spec:35`/`:273`/`:277` 三句自陈不变式（「只改容器、不改 spelling 与 meaning」）冲突——**与兼容性无关，D6 管不到**；§4 宿主对象修法实为**四处**改动（`Binder_Expressions.vb:2611-2612` 同时把守前序提交与宿主对象两条分支 + 降级侧两处 `Debug.Assert(Not _topMethod.IsShared)`）；§2「B 比 ①a 省」的唯一支柱被源码削弱（`SourceMethodSymbol.vb:82` 处 `syntax` 与 `container` 同作用域）；Motivation 的经典 VBScript 类比其自身调查材料已标「无仓内锚点、不建议作为论据」。**复活条件（缺一不得上会）**：重写 §6 机制句 / 把「零差异」降级进 Drawbacks / 按四处重定价 §4。**推荐替代路线**：维持实例基线（Active）+ issue 04 补诊断（顶层 `<Extension>` 未写 `Shared` 走 `SourceMethodSymbol.vb:1504`/`:1634` 两处断言终止）+ 甲 + 乙。见 `../meetings/inactive/meeting-top-level-implicit-shared.md`；**正面资产仍可独立用于 plan**（§7 ref struct 空结果 / §9 两模式分野 / §8 只读无硬墙与自纠 / §12 测试策略） |
