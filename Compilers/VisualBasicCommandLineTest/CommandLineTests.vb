...
Namespace Microsoft.CodeAnalysis.VisualBasic.CommandLine.UnitTests
    ...
    Partial Public Class CommandLineTests
        ...
        Private Shared ReadOnly s_basicCompilerAssembly As String = Path.ChangeExtension(s_basicCompilerExecutable, ".dll")
        ...
        Private Shared ReadOnly s_compilerVersion As String = CommonCompiler.GetProductVersion(GetType(VisualBasicCompiler))
...
End Class")
        ...
        Public Sub CompilerBinariesAreAnyCPU()
            Using stream = File.OpenRead(s_basicCompilerAssembly)
                Using peReader As New PEReader(stream)
                    Assert.Equal(Machine.I386, peReader.PEHeaders.CoffHeader.Machine)
                    Assert.True((peReader.PEHeaders.CorHeader.Flags And CorFlags.ILOnly) <> 0)
                    Assert.True((peReader.PEHeaders.CorHeader.Flags And CorFlags.Requires32Bit) = 0)
                End Using
            End Using
        End Sub
...
End Class
        ...
        Public Sub SdkPathAndLibEnvVariable()
            Dim parsedArgs = DefaultParse({"/libpath:c:lib2", "/sdkpath:c:invalid,d:\sdk1", "/vbruntime*", "/nostdlib", "a.vb"}, _baseDirectory)
...
vbc : error BC2017: could not find library 'abc'
            ...
            newVbCore.WriteAllBytes(File.ReadAllBytes(Path.Combine(s_defaultSdkDirectory, "Microsoft.VisualBasic.dll")))
...
vbc : error BC2017: could not find library 'Microsoft.VisualBasic.dll'
            ...
            msCorLib.WriteAllBytes(File.ReadAllBytes(Path.Combine(s_defaultSdkDirectory, "mscorlib.dll")))

            ' NOT: both libraries exist, but 'System.Runtime.dll' is invalid, so we need to pick up 'mscorlib.dll'
            output = ProcessUtilities.RunAndGetOutput(s_basicCompilerExecutable, "/nologo /nostdlib /sdkpath:" & dir.Path & " /t:library /vbruntime* /r:" & Path.Combine(s_defaultSdkDirectory, "System.dll") & " " & src.ToString(), startFolder:=dir.Path)
...
End Class
            ...
            Dim cmd As String = " /nologo /preferreduilang:en /sdkpath:" & sdkMultiPath &
                       " /t:library /r:" & Path.Combine(s_defaultSdkDirectory, "System.dll") &
                      " " & src.ToString()
...
Class C
      ~
        ...
        Private Sub AssertReferencePathsEqual(refPaths As ImmutableArray(Of String), sdkPathOrNothing As String, ParamArray paths() As String)
            ...
            Assert.Equal(If(sdkPathOrNothing, s_defaultSdkDirectory), refPaths(0))
...
Imports System
        ...
        Public Sub DefaultResponseFile()
            ...
            Dim corlibDir = s_defaultSdkDirectory
